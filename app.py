...
def add_business_days(start_date, business_days):
    """Return a date advanced by business_days, skipping Saturday and Sunday."""
    current = start_date
    added = 0
...
def extract_pdf(pdf_bytes):
    ...
    for l in product_lines:
        # Los nombres largos pueden venir partidos en varias líneas en el PDF.
        # Acumulamos las líneas hasta encontrar la cantidad al final.
        if re.fullmatch(r'producto\s+cant\.?', l, re.I):
            continue
        # Caso normal: nombre + cantidad en la misma línea.
        mm=re.match(r'^(.*?)(?:\s+)(\d+)\s*$',l)
        if mm:
            name_part=mm.group(1).strip()
            qty=int(mm.group(2))
            parts = pending_name + ([name_part] if name_part else [])
            if parts:
                name=' '.join(parts)
                display_name=' <br/> '.join(parts)
                pending_name=[]
                items.append({'name':name,'display_name':display_name,'qty':qty})
            else:
                # Una línea que solo contiene una cantidad no puede ser un producto por sí sola.
                continue
        # Caso de PDF con el nombre en una(s) línea(s) y la cantidad en una línea aparte.
        elif pending_name and re.fullmatch(r'\d+', l):
            qty=int(l)
            name=' '.join(pending_name)
            display_name=' <br/> '.join(pending_name)
            pending_name=[]
            items.append({'name':name,'display_name':display_name,'qty':qty})
        else:
            pending_name.append(l)
    # Customer block
...
def match_items(order_items, catalog):
    ...
    for it in order_items:
        ...
        if len(matches)==1:
            p=matches[0]; resolved.append({**it,'product_id':p['id'],'catalog_name':p['name'],'display_name':it.get('display_name',p['name']),'weight_kg':p['weight_kg'],'price':p['price'],'raw_data':p['raw_data']})
        elif len(matches)>1:
...
def pack_items(resolved,country):
    ...
    for idx,it in enumerate(resolved):
        for u in range(it['qty']): units.append({'line':idx,'name':it['catalog_name'],'display_name':it.get('display_name',it['catalog_name']),'weight':float(it['weight_kg']),'unit':u+1})
            ...
            for j in range(i+1,len(boxes)):
                for a in list(boxes[i]['units']):
                    for b in list(boxes[j]['units']):
                        # Use object identity instead of list.remove().
                        # A previous accepted swap can move an object while the
                        # outer snapshot is still being iterated, which caused
                        # intermittent ValueError crashes on some orders.
                        ai=next((k for k,x in enumerate(boxes[i]['units']) if x is a), None)
                        bj=next((k for k,x in enumerate(boxes[j]['units']) if x is b), None)
                        ...
                        if ni<=max_product+1e-9 and nj<=max_product+1e-9:
                            boxes[i]['units'][ai], boxes[j]['units'][bj] = b, a
                            ...
                            if s+1e-9 < base:
                                ...
                                # Revert using the same positions, so no
                                # equality-based remove() can fail.
                                boxes[i]['units'][ai], boxes[j]['units'][bj] = a, b
...
def summarize_box(box):
    ...
    for u in box['units']:
        key=u['name']
        grouped.setdefault(key, {'qty':0,'display_name':u.get('display_name',key)})
        grouped[key]['qty'] += 1
    return sorted([(v['display_name'], v['qty']) for v in grouped.values()], key=lambda x:x[0])
...
def format_dispatch_date_es(dispatch):
    """Format a dispatch date with the weekday in Spanish."""
    weekdays = [
        'Lunes', 'Martes', 'Miércoles', 'Jueves',
        'Viernes', 'Sábado', 'Domingo'
    ]
    d = dispatch.to_pydatetime().date() if hasattr(dispatch, 'to_pydatetime') else dispatch
...
def header_table(kind,order,box=None,dispatch=None):
    ...
    right=[Paragraph(kind,ParagraphStyle('h',fontSize=17,leading=20,alignment=TA_RIGHT,fontName='Helvetica-Bold')),
           ...
           Paragraph((f'<b>Despacho máximo: {format_dispatch_date_es(dispatch)}</b>' if dispatch else ''), ParagraphStyle('dispatch_header',fontSize=12.5,leading=15,alignment=TA_RIGHT,fontName='Helvetica-Bold'))]
...
def product_display_html(name):
    """Keep long product names in one table cell and allow explicit line breaks."""
    from xml.sax.saxutils import escape
    value=str(name)
    if '<br/>' in value:
        return value
    if len(value) > 58:
        pos=value.rfind(' ',35,59)
        if pos > 0:
            return escape(value[:pos]) + '<br/>' + escape(value[pos+1:])
    return escape(value)
...
def packing_pdf(order,box_idx,total_boxes,customer,phone,address,country,max_date,box,resolved):
    ...
    for name,qty in summarize_box(box): data.append([Paragraph(product_display_html(name),small),str(qty),CheckBox(),CheckBox()])
...
def invoice_pdf(order,customer,phone,address,country,resolved):
    ...
    for x in resolved:
        ...
        data.append([Paragraph(product_display_html(x.get('display_name',x['catalog_name'])),small),str(x['qty']),f'{price:,.0f}',f'{total:,.0f}'])
            ...
            if any(b['weight']>max_product+1e-9 for b in boxes):
                ...
                if st.button('📄 Generar documentos',type='primary'):
                    ...
                    for i,b in enumerate(boxes,1):
                        # Se despacha una sola caja por día hábil:
                        # caja 1 usa la fecha indicada por el usuario,
                        # caja 2 el siguiente día hábil, etc.
                        box_dispatch_date = add_business_days(max_date, i - 1)
                        files[f'Packing List caja {i}.pdf'] = packing_pdf(
                            order['order_num'], i, len(boxes), customer, phone,
                            address_lines, country, box_dispatch_date, b, resolved
                        )
